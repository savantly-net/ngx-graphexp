...
import {gremlinZer, renderChain} from 'zer';
...
export class GremlinClient extends EventEmitter<GremlinEvent> {
  ...
  createConnection({port, host, path, ssl, rejectUnauthorized}) {
    ...
    connection.subscribe((event: GremlinEvent) => {
      switch (event.name) {
        case 'open':
          this.onConnectionOpen();
          break;
        case 'error':
          this.handleError(event.value);
          break;
        case 'message':
          this.handleProtocolMessage(event.value);
          break;
        case 'close':
          this.handleDisconnection(event.value);
          break;
      }
    });
  ...
  warn(code, message) {
    this.emit(new GremlinEvent('warning', {
      code,
      message,
    }));
  }
  ...
  onConnectionOpen() {
    this.connected = true;
    this.emit(new GremlinEvent('connect'));
   ...
   */
  buildMessage(rawScript, rawBindings = {}, baseMessage: any = {}) {
    const {gremlin, bindings} = Utils.buildQueryFromSignature(
      rawScript,
      rawBindings,
    );
  ...
  buildChallengeResponse(requestId) {
    const {processor, op, accept, language, aliases} = this.options;
    const saslbase64 = new Buffer('\0' + this.user + '\0' + this.password).toString('base64');
